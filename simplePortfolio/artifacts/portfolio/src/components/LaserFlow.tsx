...
export function LaserFlow({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const tCtx = tc.getContext("webgl");
      if (!tCtx) return;
      tCtx.getExtension("WEBGL_lose_context")?.loseContext();
    } catch { return; }
    ...
    const material = new THREE.RawShaderMaterial({
      vertexShader: VERT,
      fragmentShader: FRAG,
      uniforms: uniforms as THREE.RawShaderMaterial["uniforms"],
      transparent: false,
      depthTest: false,
      depthWrite: false,
    ...
    const adjustDpr = (now: number) => {
      ...
      if (avg < 50) next = Math.max(currentDprRef.current * 0.85, dprFloor);
      else if (avg > 58 && currentDprRef.current < base) next = Math.min(currentDprRef.current * 1.1, base);
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || "#FF6600");
  ...
  return (
    <div
      ref={mountRef}
      className={className}
      style={{ width: "100%", height: "100%", position: "relative", pointerEvents: "none", ...style }}
    />
  );
}
